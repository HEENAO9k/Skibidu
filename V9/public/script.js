...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Added: Initialize theme and notifications
  initializeTheme();
...
function updateUIFromConfig(config) {
  ...
  // Added: update theme from server-side preference (optional)
  if (config.theme && (config.theme === 'light' || config.theme === 'dark')) {
    document.documentElement.setAttribute('data-theme', config.theme);
    try { localStorage.setItem('theme-preference', config.theme); } catch(_) {}
  }
}
...
// [Added] Theme initialization with persistence using localStorage
...
// [Added] Request permission and show notification when new events arrive
function initializeNotification() {
  ...
  // Hook existing socket 'new-event' to also trigger native notification
  try {
    ...
    socket.on('new-event', (event) => {
      ...
      if (Notification.permission === 'granted') {
        const n = new Notification('อีเว้นท์ใหม่', {
          body: event && event.title ? event.title : 'มีข้อมูลใหม่เข้ามา',
          icon: '/favicon.ico'
        });
...
// Added: Global search highlighter on index page (client-side)
(function setupGlobalSearch(){
  const input = document.getElementById('globalSearch');
  if (!input) return;
  const fields = [
    'textureName','youtubeUrl','youtubeAudioUrl','tiktokUrl','tiktokAudioUrl','email'
  ].map(id => document.getElementById(id)).filter(Boolean);

  function highlight(el, on) {
    if (!el) return;
    el.style.outline = on ? '2px solid var(--accent-color)' : '';
    el.style.boxShadow = on ? '0 0 0 3px rgba(240, 147, 251, 0.25)' : '';
  }

  input.addEventListener('input', () => {
    const q = input.value.trim().toLowerCase();
    if (!q) {
      fields.forEach(el => highlight(el, false));
      return;
    }
    fields.forEach(el => {
      const value = (el.value || '').toLowerCase();
      const placeholder = (el.placeholder || '').toLowerCase();
      const matched = value.includes(q) || placeholder.includes(q);
      highlight(el, matched);
    });
  });
})();
