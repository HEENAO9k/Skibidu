...
class ThemeManager {
  ...
  toggleTheme() {
    ...
    NotificationManager.addNotification(
      'เปลี่ยนธีม',
      `เปลี่ยนเป็นโหมด ${newTheme === 'dark' ? 'มืด' : 'สว่าง'} แล้ว`,
      'info'
    );
  }
...
class SearchSystem {
  ...
  async loadSearchData() {
    try {
      // Load sample search data (in real app, this would come from API)
      this.searchData = [
        {
          id: 1,
          title: 'Minecraft Grass Texture',
          description: 'Texture pack สำหรับบล็อกหญ้าแบบธรรมชาติ',
          type: 'texture',
          tags: ['grass', 'nature', 'minecraft']
        },
        {
          id: 2,
          title: 'Stone Block Pack',
          description: 'ชุด texture สำหรับบล็อกหินแบบเรียบง่าย',
          type: 'texture',
          tags: ['stone', 'simple', 'blocks']
        },
        {
          id: 3,
          title: 'Wood Texture Collection',
          description: 'คอลเลกชัน texture ไม้หลากหลายชนิด',
          type: 'texture',
          tags: ['wood', 'collection', 'natural']
        },
        {
          id: 4,
          title: 'Water Animation Pack',
          description: 'Animation สำหรับบล็อกน้ำแบบเคลื่อนไหว',
          type: 'animation',
          tags: ['water', 'animation', 'fluid']
        },
        {
          id: 5,
          title: 'Fire Effect Texture',
          description: 'Texture สำหรับเอฟเฟกต์ไฟและลาวา',
          type: 'effect',
          tags: ['fire', 'lava', 'effect']
        }
      ];
    } catch (error) {
    ...
    if (results.length === 0) {
      this.searchResults.innerHTML = `
        <div class="search-result-item">
          <div class="search-result-title">ไม่พบผลลัพธ์</div>
          <div class="search-result-description">ลองค้นหาด้วยคำอื่น</div>
        </div>
      `;
    } else {
      this.searchResults.innerHTML = results.map(item => `
        <div class="search-result-item" data-id="${item.id}">
          ...
          <div class="search-result-description">${this.highlightMatch(item.description)}</div>
        </div>
      `).join('');
    }
  ...
  selectSearchResult(id) {
    const selectedItem = this.searchData.find(item => item.id == id);
    if (selectedItem) {
      // Fill search input with selected item
      this.searchInput.value = selectedItem.title;
      
      // Show notification
      NotificationManager.addNotification(
        'เลือกผลการค้นหา',
        `เลือก "${selectedItem.title}" แล้ว`,
        'success'
      );
      
      // In a real app, you might navigate to the item or show details
...
class NotificationManager {
  ...
  static setupEventListeners() {
    ...
    document.addEventListener('click', (e) => {
      if (!this.notificationBell.contains(e.target) && !this.notificationPanel.contains(e.target)) {
  ...
  static addNotification(title, message, type = 'info', autoRemove = true) {
    const notification = {
      id: Date.now(),
      title,
      message,
    ...
    if (this.notifications.length === 0) {
      this.notificationList.innerHTML = `
        <div class="notification-item">
          <div class="notification-title">ไม่มีการแจ้งเตือน</div>
          <div class="notification-message">คุณจะเห็นการแจ้งเตือนใหม่ที่นี่</div>
        </div>
      `;
    } else {
  ...
  static showNotificationPanel() {
    this.notificationPanel.style.display = 'block';
  }

  static hideNotificationPanel() {
  ...
  static startDemoNotifications() {
    const demoMessages = [
      { title: 'Texture Pack พร้อมแล้ว', message: 'Minecraft Grass Pack ของคุณประมวลผลเสร็จแล้ว', type: 'success' },
      { title: 'อัปเดตระบบ', message: 'ระบบได้รับการอัปเดตเป็นเวอร์ชัน 3.1', type: 'info' },
      { title: 'การแจ้งเตือนใหม่', message: 'มี Texture Pack ใหม่ที่คุณอาจสนใจ', type: 'info' }
    ];

    let index = 0;
    setInterval(() => {
      const demo = demoMessages[index % demoMessages.length];
      this.addNotification(demo.title, demo.message, demo.type, false);
      index++;
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  setTimeout(() => {
    NotificationManager.startDemoNotifications();
  }, 5000);
  ...
  setTimeout(() => {
    NotificationManager.addNotification(
      'ยินดีต้อนรับ!',
      'ระบบค้นหาและแจ้งเตือนพร้อมใช้งานแล้ว',
      'success',
      false
    );
  }, 1000);
