...
io.on('connection', (socket) => {
  ...
  // ส่งการแจ้งเตือนต้อนรับผู้ใช้ใหม่
  socket.emit('notification', {
    type: 'success',
    title: 'เชื่อมต่อสำเร็จ!',
    message: 'ยินดีต้อนรับสู่ BetMC Texture Generator Pro',
    timestamp: new Date().toISOString()
  });

  // ส่งข้อมูลการตรวจสอบล่าสุด
  socket.emit('monitoring-update', {
    newDataAvailable: dataMonitoring.newDataAvailable,
    lastCheck: dataMonitoring.lastProcessTime
  });

  // รับคำขอข้อมูลการตรวจสอบ
  socket.on('request-monitoring-data', () => {
    socket.emit('monitoring-update', {
      newDataAvailable: dataMonitoring.newDataAvailable,
      lastCheck: dataMonitoring.lastProcessTime,
      recentActivity: dataMonitoring.recentActivity.slice(0, 10)
    });
  });

  // รับคำขอล้างการแจ้งเตือน
  socket.on('clear-notifications', () => {
    dataMonitoring.recentActivity = dataMonitoring.recentActivity.filter(
      activity => activity.type !== 'notification'
    );
...
// ========================================================================================
// ENHANCED FEATURES: Notification System and Real-time Data Monitoring
// ========================================================================================

/**
 * Enhanced Notification System
 * ส่งการแจ้งเตือนแบบ Real-time เมื่อมีข้อมูลใหม่หรือเหตุการณ์สำคัญ
 */

// ระบบติดตามข้อมูลใหม่
...
function broadcastNotification(type, title, message, targetRoom = null) {
  ...
  // บันทึกการแจ้งเตือนในประวัติ
  dataMonitoring.recentActivity.unshift({
    type: 'notification',
    data: notification,
    timestamp: Date.now()
  });
  ...
  // จำกัดจำนวนประวัติ
  if (dataMonitoring.recentActivity.length > dataMonitoring.maxActivityHistory) {
    dataMonitoring.recentActivity = dataMonitoring.recentActivity.slice(0, dataMonitoring.maxActivityHistory);
  }

  console.log(`📢 Notification sent: [${type.toUpperCase()}] ${title} - ${message}`);
}

/**
 * ส่งการแจ้งเตือนความคืบหน้า
 */
function sendProgressNotification(sessionId, step, message, percentage = null) {
  const progressData = {
    sessionId: sessionId,
    step: step,
    message: message,
  ...
  // ส่งไปยัง room เฉพาะของ session
  io.to(`progress-${sessionId}`).emit('progress-notification', progressData);
  
  // ส่งไปยัง admin room ด้วย
  io.to('admin-room').emit('progress-notification', progressData);

  console.log(`📊 Progress notification: ${step} - ${message} (${percentage || 'N/A'}%)`);
}

/**
 * ติดตามข้อมูลใหม่และส่งการแจ้งเตือน
 */
function monitorNewData() {
  // ตรวจสอบไฟล์ใหม่ในโฟลเดอร์ uploads
  ...
  if (fs.existsSync(uploadPath)) {
    fs.readdir(uploadPath, (err, files) => {
      ...
      const recentFiles = files.filter(file => {
        ...
        const stats = fs.statSync(filePath);
        return stats.mtime.getTime() > dataMonitoring.lastProcessTime;
      });

      if (recentFiles.length > 0) {
        dataMonitoring.newDataAvailable = true;
        
        // ส่งการแจ้งเตือนข้อมูลใหม่
        io.emit('new-data-available', {
          message: `พบไฟล์ใหม่ ${recentFiles.length} ไฟล์`,
          files: recentFiles,
          timestamp: Date.now()
        });
        ...
        broadcastNotification(
          'info',
          'ข้อมูลใหม่!',
          `มีไฟล์ใหม่ ${recentFiles.length} ไฟล์ พร้อมสำหรับการประมวลผล`
        );
      }
    });
  }
  ...
  // แจ้งเตือนเมื่อใช้ทรัพยากรสูง
  if (memoryUsage.heapUsed / memoryUsage.heapTotal > 0.8) {
    broadcastNotification(
      'warning',
      'การใช้หน่วยความจำสูง',
      'ระบบกำลังใช้หน่วยความจำมาก โปรดรอสักครู่',
      'admin-room'
    );
  }

  dataMonitoring.lastProcessTime = Date.now();
}

// ตั้งเวลาตรวจสอบข้อมูลใหม่ทุก 30 วินาที
setInterval(monitorNewData, 30000);
...
app.get('/api/notifications/history', (req, res) => {
  const notifications = dataMonitoring.recentActivity
...
// ส่งการแจ้งเตือนแบบ manual (สำหรับ admin)
app.post('/api/notifications/send', (req, res) => {
  const { type, title, message, targetRoom } = req.body;
  ...
  broadcastNotification(type, title, message, targetRoom);
...
// ดึงสถานะการติดตามข้อมูล
app.get('/api/monitoring/status', (req, res) => {
  res.json({
    success: true,
    monitoring: {
      lastProcessTime: dataMonitoring.lastProcessTime,
      newDataAvailable: dataMonitoring.newDataAvailable,
      recentActivityCount: dataMonitoring.recentActivity.length,
      systemStatus: {
...
/**
 * Enhanced Socket.IO Events สำหรับระบบแจ้งเตือน
 * (เพิ่มเข้าไปในระบบ socket.io ที่มีอยู่แล้ว)
 */
// Note: จะรวมเข้ากับ socket.io connection handler ที่มีอยู่แล้ว

/**
 * Enhanced Error Handling และ Logging
 */

// ปรับปรุงฟังก์ชัน addErrorLog ให้ส่งการแจ้งเตือนด้วย
const originalAddErrorLog = addErrorLog;
addErrorLog = function(type, message, details = null) {
  // เรียกฟังก์ชันเดิม
  originalAddErrorLog(type, message, details);
  
  // ส่งการแจ้งเตือนตามประเภท
  let notificationType = 'info';
  let notificationTitle = 'ข้อมูลระบบ';
  
  switch(type) {
    case 'error':
      notificationType = 'error';
      notificationTitle = 'เกิดข้อผิดพลาด';
      break;
    case 'warning':
      notificationType = 'warning';
      notificationTitle = 'คำเตือน';
      break;
    case 'success':
      notificationType = 'success';
      notificationTitle = 'สำเร็จ';
      break;
  }
  
  // ส่งการแจ้งเตือนไปยัง admin room เท่านั้น
  if (type === 'error' || type === 'warning') {
    broadcastNotification(notificationType, notificationTitle, message, 'admin-room');
  } else if (type === 'success' && message.includes('สร้าง') && message.includes('เสร็จ')) {
    // แจ้งเตือนเมื่อสร้าง texture เสร็จ
    broadcastNotification(notificationType, notificationTitle, message);
  }
};
...
function startPerformanceMonitoring() {
  setInterval(() => {
    ...
    // ส่งข้อมูลประสิทธิภาพไปยัง admin
    io.to('admin-room').emit('performance-realtime', {
      memory: {
...
// เริ่มต้นการติดตามประสิทธิภาพ
startPerformanceMonitoring();

/**
 * Enhanced File Processing Notifications
 */

// ปรับปรุงฟังก์ชันประมวลผลไฟล์ให้ส่งการแจ้งเตือนแบบ real-time
function enhanceProcessingWithNotifications() {
  // Hook เข้าไปในกระบวนการประมวลผลหลัก
  const originalEmitProgress = (sessionId, step, message, percentage) => {
    // ส่ง progress แบบเดิม
    io.to(`progress-${sessionId}`).emit('progress-update', {
      step: step,
      message: message,
      percentage: percentage
    });
    ...
    // ส่งการแจ้งเตือนความคืบหน้า
    sendProgressNotification(sessionId, step, message, percentage);
    ...
    if (percentage >= 100) {
      setTimeout(() => {
        broadcastNotification(
          'success',
          'ประมวลผลเสร็จสิ้น!',
          'Texture Pack ของคุณพร้อมดาวน์โหลดแล้ว'
        );
      }, 1000);
    }
  };
  
  // Export ฟังก์ชันให้ส่วนอื่นใช้งาน
  global.emitProgress = originalEmitProgress;
}

enhanceProcessingWithNotifications();
